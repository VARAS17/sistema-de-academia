<div class="py-8 bg-gray-50 min-h-screen font-sans antialiased text-gray-900">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <!-- 2. MENSAJES DE ESTADO (Visibilidad del sistema) -->
                    ...
                    <div class="overflow-x-auto border border-gray-50 rounded-2xl">
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse($simulacros as $sim)
                                    <tr class="hover:bg-indigo-50/30 transition-colors group">
                                        <td class="px-6 py-4">
                                            ...
                                            <div class="text-[10px] text-gray-400 font-bold uppercase tracking-tight">Puntaje Máx: {{ $sim->puntaje_maximo }}</div>
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex justify-center space-x-2">
                                                <button wire:click="showEdit({{ $sim->id }})" class="p-2 text-indigo-600 hover:bg-indigo-100 rounded-lg transition-colors" title="Editar">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                </button>
                                                <button onclick="confirm('¿Desea eliminar este simulacro?') || event.stopImmediatePropagation()" 
                                                        wire:click="delete({{ $sim->id }})" class="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-colors" title="Eliminar">
                                ...
                                @empty
                                    <tr><td colspan="5" class="px-6 py-12 text-center text-gray-400 italic">No se encontraron simulacros registrados.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="p-8 max-w-4xl mx-auto animate-fade-in">
                    <div class="flex items-center justify-between mb-8 border-b border-gray-100 pb-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">Configure los parámetros del examen de simulacro.</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div class="md:col-span-2 space-y-2">
                                ...
                                <input type="text" wire:model="nombre" 
                                       class="w-full p-3 bg-gray-50 border-2 border-gray-50 rounded-xl focus:border-indigo-500 focus:bg-white transition-all outline-none font-bold text-gray-700 @error('nombre') border-rose-200 bg-rose-50 @enderror" 
                                       placeholder="Ej: Simulacro Tipo Admisión - Fase I">
                                @error('nombre') <p class="text-rose-500 text-[10px] font-bold uppercase mt-1 ml-1 tracking-tight">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <select wire:model.live="area_id" 
                                        class="w-full p-3 bg-gray-50 border-2 border-gray-50 rounded-xl focus:border-indigo-500 focus:bg-white transition-all outline-none font-bold text-sm cursor-pointer">
                            ...
                            <!-- Ciclo (Heurística #5: Prevención de errores) -->
                            <div class="space-y-2">
                                ...
                                <select wire:model="ciclo_id" 
                                        class="w-full p-3 bg-gray-50 border-2 border-gray-50 rounded-xl focus:border-indigo-500 focus:bg-white transition-all outline-none font-bold text-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" 
                                        {{ !$area_id ? 'disabled' : '' }}>
                                ...
                                @if(!$area_id) <p class="text-indigo-400 text-[9px] font-bold uppercase mt-1 tracking-tighter">← Seleccione primero el área académica.</p> @endif
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="date" wire:model="fecha" 
                                       class="w-full p-3 bg-gray-50 border-2 border-gray-50 rounded-xl focus:border-indigo-500 focus:bg-white transition-all outline-none font-bold text-gray-700">
                                @error('fecha') <p class="text-rose-500 text-[10px] font-bold uppercase mt-1 ml-1">{{ $message }}</p> @enderror
                            </div>

                            <!-- Puntaje Máximo -->
                            <div class="space-y-2">
                                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Puntaje Máximo Posible</label>
                                <input type="number" wire:model="puntaje_maximo" 
                                       class="w-full p-3 bg-gray-50 border-2 border-gray-50 rounded-xl focus:border-indigo-500 focus:bg-white transition-all outline-none font-bold text-gray-700" 
                                       placeholder="Ej: 400.00">
                                @error('puntaje_maximo') <p class="text-rose-500 text-[10px] font-bold uppercase mt-1 ml-1">{{ $message }}</p> @enderror
                            </div>

                        </div>

                        <!-- Botones de Acción (Heurística #3: Control y Libertad) -->
