<div class="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen font-sans antialiased text-gray-900">
    ...
    <!-- 1. SECCIÓN DE BREADCRUMBS (Heurística: Reconocimiento antes que recuerdo) -->
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            @foreach($breadcrumbs as $bc)
                <li class="inline-flex items-center">
                    ...
                    @endif
                    <a href="{{ $bc['url'] ?? '#' }}" class="inline-flex items-center text-sm font-semibold transition-colors {{ $bc['url'] ? 'text-gray-500 hover:text-indigo-600' : 'text-indigo-600 cursor-default' }}">
                        @if($loop->first)
    ...
    <!-- 3. BARRA DE FILTROS (Heurística: Flexibilidad y Eficiencia) -->
    @role('admin')
    <div class="mb-8 grid grid-cols-1 md:grid-cols-3 gap-6 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        <div class="space-y-1">
        ...
        <div class="space-y-1">
            ...
            <select wire:model.live="filtro_ciclo" class="w-full bg-gray-50 border-2 border-gray-50 rounded-xl font-bold text-sm focus:bg-white focus:border-indigo-500 transition-all outline-none p-3 cursor-pointer {{ empty($ciclos_filtro) ? 'opacity-50' : '' }}" {{ empty($ciclos_filtro) ? 'disabled' : '' }}>
    ...
    <!-- 4. MENSAJES DE ESTADO (Heurística: Visibilidad) -->
    ...
    <!-- 5. GRID DE HORARIOS (Heurística: Diseño Estético) -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        @empty
            <div class="col-span-full flex flex-col items-center justify-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-gray-100">
                <div class="h-20 w-20 bg-gray-50 rounded-full flex items-center justify-center text-gray-200 mb-4">
    ...
    <!-- 6. MODAL PARA CREAR/EDITAR (HCI: Control del Usuario) -->
    @if($isModalOpen)
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
        <!-- Backdrop Blur -->
        <div class="fixed inset-0 bg-indigo-950/40 backdrop-blur-sm transition-opacity"></div>

        <!-- Modal Card -->
        <div class="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl transform transition-all overflow-hidden border border-gray-100">
            <div class="p-8">
                ...
                <div class="space-y-6">
                    ...
                    <!-- Imagen (Dropzone style) -->
                    <div class="space-y-1">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Cargar Imagen de Horario</label>
                        <div class="mt-1 flex justify-center px-6 pt-8 pb-8 border-2 border-gray-100 border-dashed rounded-3xl bg-gray-50 hover:bg-indigo-50/50 hover:border-indigo-300 transition-all group relative">
                            <div class="space-y-2 text-center">
                                @if ($imagen)
                                    <img src="{{ $imagen->temporaryUrl() }}" class="mx-auto h-40 w-auto rounded-2xl shadow-xl mb-4">
                                @elseif($horario_id)
                                    <p class="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-4 italic">Se mantendrá la imagen actual si no elige una nueva.</p>
                                @else
                                    <div class="mx-auto h-16 w-16 bg-white rounded-2xl shadow-sm flex items-center justify-center text-gray-300 group-hover:text-indigo-500 transition-colors">
                        ...
                        <div wire:loading wire:target="imagen" class="text-[10px] font-black text-indigo-500 mt-2 animate-pulse uppercase tracking-widest italic text-center">Subiendo imagen...</div>
                        @error('imagen') <p class="text-rose-500 text-[10px] font-black uppercase mt-1 ml-1 tracking-tight">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="mt-12 flex flex-col sm:flex-row justify-end gap-4 border-t border-gray-50 pt-8">
                    <button wire:click="closeModal" class="px-8 py-3 text-xs font-black text-gray-400 hover:text-gray-600 uppercase tracking-widest transition-colors">
                        Cancelar
                    </button>
                    <button wire:click="save" wire:loading.attr="disabled" class="px-10 py-3.5 bg-indigo-600 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all active:scale-95 flex items-center justify-center">
